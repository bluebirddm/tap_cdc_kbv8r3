...
public class PostgresValueConverter extends JdbcValueConverters {
    ...
    private static final Pattern INTERVAL_YEARS = Pattern.compile("([-+]?\\d+)\\s+years?");
    private static final Pattern INTERVAL_MONTHS = Pattern.compile("([-+]?\\d+)\\s+mons?");
    private static final Pattern INTERVAL_DAYS = Pattern.compile("([-+]?\\d+)\\s+days?");
    private static final Pattern INTERVAL_TIME = Pattern.compile("([-+]?\\d{1,2}):(\\d{2}):(\\d{2})(?:\\.(\\d+))?");
    ...
    protected Object convertInterval(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, NumberConversions.LONG_FALSE, (r) -> {
            if (data instanceof Number) {
                ...
                final IntervalComponents interval = parseIntervalString((String) data);
                if (interval != null) {
                    if (intervalMode == IntervalHandlingMode.STRING) {
                        r.deliver(
                        ...
                        r.deliver(
                                MicroDuration.durationMicros(
                                        ...
                                        interval.hours,
                                        interval.minutes,
                                        interval.seconds.doubleValue(),
                                        MicroDuration.DAYS_PER_MONTH_AVG));
                    }
                }
                else if (intervalMode == IntervalHandlingMode.STRING) {
                    r.deliver(data);
                }
            }
        });
    ...
    private static IntervalComponents parseIntervalString(String value) {
        if (value == null) {
            return null;
        }
        ...
        final int years = extractIntervalPart(INTERVAL_YEARS, trimmed);
        final int months = extractIntervalPart(INTERVAL_MONTHS, trimmed);
        final int days = extractIntervalPart(INTERVAL_DAYS, trimmed);

        int hours = 0;
        int minutes = 0;
        BigDecimal seconds = BigDecimal.ZERO;

        final Matcher timeMatcher = INTERVAL_TIME.matcher(trimmed);
        if (timeMatcher.find()) {
            final String hourPart = timeMatcher.group(1);
            final boolean negative = hourPart.startsWith("-");
            hours = Integer.parseInt(hourPart);
            minutes = Integer.parseInt(timeMatcher.group(2));
            final String secondPart = timeMatcher.group(3);
            final String fraction = timeMatcher.group(4);
            seconds = new BigDecimal(secondPart + (fraction != null ? "." + fraction : ""));
            if (negative) {
                minutes = -minutes;
        ...
        return new IntervalComponents(years, months, days, hours, minutes, seconds);
    }

    private static int extractIntervalPart(Pattern pattern, String value) {
    ...
    private static final class IntervalComponents {
        ...
        private IntervalComponents(int years, int months, int days, int hours, int minutes, BigDecimal seconds) {
            this.years = years;
            this.months = months;
            this.days = days;
            this.hours = hours;
