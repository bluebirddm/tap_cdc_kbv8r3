...
public class PostgresConnection extends JdbcConnection {
    ...
    public ServerInfo.ReplicaIdentity readReplicaIdentityInfo(TableId tableId) throws SQLException {
        String statement = "SELECT relreplident FROM sys_catalog.sys_class c " +
                "LEFT JOIN sys_catalog.sys_namespace n ON c.relnamespace=n.oid " +
    ...
    protected ServerInfo.ReplicationSlot queryForSlot(String slotName, String database, String pluginName,
            ...
            throws SQLException {
        final String slotQuery;
        DatabaseMetaData metadata = connection().getMetaData();
        if ("KingbaseES".equalsIgnoreCase(metadata.getDatabaseProductName())) {
            slotQuery = "select * from sys_catalog.sys_replication_slots where slot_name = ? and database = ? and plugin = ?";
        }
        else {
            slotQuery = "select * from pg_replication_slots where slot_name = ? and database = ? and plugin = ?";
        }

        return prepareQueryAndMap(slotQuery, statement -> {
    ...
    public boolean dropReplicationSlot(String slotName) {
        ...
        for (int i = 0; i < ATTEMPTS; i++) {
            try {
                DatabaseMetaData metadata = connection().getMetaData();
                String dropSql = "KingbaseES".equalsIgnoreCase(metadata.getDatabaseProductName())
                        ? "select sys_drop_replication_slot('" + slotName + "')"
    ...
    public long currentXLogLocation() throws SQLException {
        ...
        final String lsnQuery;
        if ("KingbaseES".equalsIgnoreCase(productName)) {
            lsnQuery = "select sys_current_xlog_location()";
        }
        else {
            lsnQuery = majorVersion >= 10 ? "select pg_current_wal_lsn()" : "select pg_current_xlog_location()";
        }
    ...
    public ServerInfo serverInfo() throws SQLException {
        ...
        boolean isKingbase = "KingbaseES".equalsIgnoreCase(connection().getMetaData().getDatabaseProductName());
        if (username != null && !isKingbase) {
    ...
    private static void validateServerVersion(Statement statement) throws SQLException {
        ...
        if ("KingbaseES".equalsIgnoreCase(productName)) {
            if (majorVersion < 8) {
                throw new SQLException("Cannot connect to a version of Kingbase lower than 8");
            }
            return;
        }
